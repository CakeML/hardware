...
Definition PC_next_def:
 PC_next (fext:ext) s s' = s' with PC_next := s.PC + 4w
End

Definition decode_instruction_def:
 decode_instruction (fext:ext) (s:state_circuit) s' =
...
Definition wV_def:
 wV (fext:ext) (s:state_circuit) s' = s' with wV := DecodeReg_imm (word_bit 31 s'.i, (30 >< 25) s'.i) s'
End

Theorem wV_trans = REWRITE_RULE [DecodeReg_imm_def] wV_def;

Definition aV_def:
 aV (fext:ext) (s:state_circuit) s' = s' with aV := DecodeReg_imm (word_bit 23 s'.i, (22 >< 17) s'.i) s'
End

Theorem aV_trans = REWRITE_RULE [DecodeReg_imm_def] aV_def;

Definition bV_def:
 bV (fext:ext) (s:state_circuit) s' = s' with bV := DecodeReg_imm (word_bit 16 s'.i, (15 >< 10) s'.i) s'
End

Theorem bV_trans = REWRITE_RULE [DecodeReg_imm_def] bV_def;
...
val execute_instruction_def = Define `
 execute_instruction fext s s' =
  case s'.instruction of
    (* Normal *)
    0w => s' with <|state := 1w; command := 1w; PC := s'.PC_next;
                   R := ((30 >< 25) s'.i =+ s'.ALU_res) s'.R|>

    (* Shift *)
  | 1w => let s' = shift ((7 >< 6) s'.i) ((30 >< 25) s'.i) s'.aV s'.bV s' in
          s' with <|state := 1w; command := 1w; PC := s'.PC_next|>

    (* StoreMEM *)
  | 2w => s' with <|state := 1w; command := 3w; PC := s'.PC_next;
                   data_addr := s'.bV; data_wdata := s'.aV; data_wstrb := 15w|>

    (* StoreMEMByte *)
    (* TODO: Can reuse shifter here? *)
  | 3w => (let s' = s' with <|state := 1w; command := 3w; PC := s'.PC_next;
                            data_addr := s'.bV; data_wstrb := 1w <<~ w2w ((1 >< 0) s'.bV)|> in
             (case (1 >< 0) s'.bV of
               0w => s' with data_wdata := bit_field_insert 7 0 ((7 >< 0) s'.aV) s'.data_wdata
             | 1w => s' with data_wdata := bit_field_insert 15 8 ((7 >< 0) s'.aV) s'.data_wdata
             | 2w => s' with data_wdata := bit_field_insert 23 16 ((7 >< 0) s'.aV) s'.data_wdata
             | 3w => s' with data_wdata := bit_field_insert 31 24 ((7 >< 0) s'.aV) s'.data_wdata))

    (* LoadMEM *)
  | 4w => s' with <|state := 1w; command := 2w; PC := s'.PC_next;
                   data_addr := s'.aV; do_delay_write := 4w; delay_write := (30 >< 25) s'.i|>

    (* LoadMEMByte *)
  | 5w => s' with <|state := 1w; command := 2w; PC := s'.PC_next;
                   data_addr := s'.aV; do_delay_write := w2w ((1 >< 0) s'.aV);
                   delay_write := (30 >< 25) s'.i|>

    (* Out *)
  | 6w => s' with <|state := 1w; command := 1w; PC := s'.PC_next;
                   data_out := (9 >< 0) s'.ALU_res; R := ((30 >< 25) s'.i =+ s'.ALU_res) s'.R|>

    (* In *)
  | 7w => s' with <|state := 1w; command := 1w; PC := s'.PC_next;
                   R := ((30 >< 25) s'.i =+ w2w s'.data_in) s'.R|>

    (* Accelerator *)
  | 8w => s' with <|state := 2w; PC := s'.PC_next;
                   acc_arg_ready := T; acc_arg := s'.aV; delay_write := (30 >< 25) s'.i|>

    (* Jump *)
  | 9w => s' with <|state := 1w; command := 1w; PC := s'.ALU_res;
                   R := ((30 >< 25) s'.i =+ s'.PC_next) s'.R|>

    (* JumpIfZero *)
  | 10w => (if s'.ALU_res = 0w then s' with PC := s.PC + s'.wV else s' with PC := s'.PC_next)
           with <|state := 1w; command := 1w|>

    (* JumpIfNotZero *)
  | 11w => (if s'.ALU_res <> 0w then s' with PC := s.PC + s'.wV else s' with PC := s'.PC_next)
           with <|state := 1w; command := 1w|>

    (* Interrupt *)
  | 12w => s' with <|state := 1w; command := 4w; data_addr := 0w; do_interrupt := T; PC := s'.PC_next|>
  ...
  | 13w => (if word_bit 23 s'.i then
            ...
            with <|state := 1w; command := 1w; PC := s'.PC_next|>
...
val cpu_Next_0w_def = Define `
 cpu_Next_0w fext s s' = let
  func = if s'.instruction = 0w ∨ s'.instruction = 6w ∨
          ...
          9w;
  ALU_fst = if s'.instruction = 9w then s.PC else s'.aV;
  ALU_snd = if s'.instruction = 9w then s'.aV else s'.bV;
  s' = ALU (func, ALU_fst, ALU_snd) s' in
 execute_instruction fext s s'`;
...
(* Some rewrites no longer needed here... *)
Theorem cpu_Next_trans = cpu_Next_def
 |> REWRITE_RULE [cpu_Next_0w_def, cpu_Next_1w_def, cpu_Next_2w_def, cpu_Next_3w_def, cpu_Next_4w_def,
                  (* 0w: *) align_addr_def, ALU_def, shift_trans, execute_instruction_def,
...
Definition ag32_def:
 ag32 = mk_module (procs [cpu_Next]) (procs [PC_next; decode_instruction; wV; aV; bV]) ag32_init
End
