...
val mem_update_def = Define `
 mem_update mem addr wdata wstrb =
  let
    oldv = mem addr;
    newv = (case wstrb of
       1w  (* 0001 *) => bit_field_insert 7 0 ((7 >< 0) wdata) oldv
     | 2w  (* 0010 *) => bit_field_insert 15 8 ((15 >< 8) wdata) oldv
     | 4w  (* 0100 *) => bit_field_insert 23 16 ((23 >< 16) wdata) oldv
     | 8w  (* 1000 *) => bit_field_insert 31 24 ((31 >< 24) wdata) oldv
     | 15w (* 1111 *) => wdata)
  in
    (addr =+ newv) mem`;
...
val execute_instruction_def = Define `
 execute_instruction wV aV bV PC_next fext s =
  ...
  | 2w => s with <|state := 1w; command := 3w; PC := PC_next;
                   data_addr := align_addr bV; data_wdata := aV; data_wstrb := 15w|>

    (* StoreMEMByte *)
    (* TODO: Can reuse shifter here? *)
  | 3w => (let s = s with <|state := 1w; command := 3w; PC := PC_next;
                            data_addr := align_addr bV; data_wstrb := 1w <<~ w2w ((1 >< 0) bV)|> in
             (case (1 >< 0) bV of
  ...
  | 4w => s with <|state := 1w; command := 2w; PC := PC_next;
                   data_addr := align_addr aV; do_delay_write := 4w; delay_write := (30 >< 25) s.i|>

    (* LoadMEMByte *)
  | 5w => s with <|state := 1w; command := 2w; PC := PC_next;
                   data_addr := align_addr aV; do_delay_write := w2w ((1 >< 0) aV);
